...
def convert_rdd(df, sc, item_id):
    ...
    df_rdd = sqlContext.createDataFrame(df[['user_id', item_id, 'is_listened']]).rdd
    df_rdd = df_rdd.map(lambda r: Rating(int(r[0]), int(r[1]), float(r[2])))
    return df_rdd

def compute_y_pred(test_f, predictions, item_id, user_dict):
    ...
    ALS_pred_df[item_id] = ALS_pred_df[item_id].astype(int)

    test_f_merged = test_f.merge(ALS_pred_df, how='left', on=['user_id', item_id])
    ...
    return test_f_merged[['user_id', item_id, 'is_listened', 'is_listened_pred']]